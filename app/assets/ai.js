...
(function (root) {
  ...
  function waiverPrefix() {
    var lines = [];
    lines.push('You are advising on fantasy football waiver-wire moves for MY roster.');
    lines.push('');
    lines.push('WHAT I WANT BACK: specific one-for-one drop/add pairs, ranked, each with the');
    lines.push('reasoning and the expected REST-OF-SEASON point edge in this league\'s');
    lines.push('scoring — or a plain statement that no move is worth making.');
    lines.push('');
    lines.push('THIS LEAGUE DOES NOT USE STANDARD SCORING. Its exact table:');
    lines.push(rulesText());
    lines.push('');
    ...
    lines.push('');
    lines.push(waiverCriteriaText());
    lines.push('');
    lines.push('WHAT THE APP HAS ALREADY WORKED OUT, AND WHAT IT HAS NOT.');
    lines.push('Settled fact: who is unrostered in this ten-team league, and what every');
    lines.push('player is worth in this scoring. No public list knows either. Every');
    lines.push('REST-OF-SEASON number below is expected points from now to the end of the');
    lines.push('regular season, built from full-season projections re-scored under the');
    lines.push('table above, blended with this season\'s measured games, times the games');
    lines.push('each man has left after his bye. Take those as the starting line.');
    lines.push('Open to you: whether they still describe reality — a depth-chart move, an');
    lines.push('injury ahead of somebody, a rookie taking a role. That is what you add.');
    lines.push('');
    lines.push('ALSO: for each player in MY ROSTER — INJURIES below who is NOT on a bye,');
    lines.push('search for his rest-of-season outlook — severity, body part, expected');
    lines.push('timeline — and report it in the "injuries" list even if you conclude he');
    lines.push('does not need replacing. The bye-week ones need no research.');
    lines.push('');
    lines.push(qbSkepticismText());
    lines.push('');
    lines.push('Answer with JSON ONLY, no prose outside it, in exactly this shape:');
    lines.push('{"swaps":[{"drop":"<exact name of ONE player on MY roster>",');
    lines.push('  "add":"<exact name of ONE player from AVAILABLE>",');
    lines.push('  "pos":"QB|RB|WR|TE|K|DEF","nfl":"<team abbr of the man being added>",');
    lines.push('  "rank":1,');
    lines.push('  "edge":<expected REST-OF-SEASON point difference, add minus drop, as a');
    lines.push('    number in this league\'s scoring — e.g. 54>,');
    lines.push('  "mandated":<true only when the drop is forced: season-ending injury,');
    lines.push('    indefinite suspension, permanently lost the job>,');
    lines.push('  "confidence":"high|medium|low",');
    lines.push('  "why":"<why this exact man beats that exact man for the rest of the');
    lines.push('    season: the role or news reason, dated, with the outlet named, and');
    lines.push('    what it means under THIS scoring. Two or three sentences.>"}],');
    ...
    lines.push('    e.g. \'3 rec, 34 yds vs DAL (Wk 2)\'. Empty string if none found.>",');
    lines.push('  "dropCandidate":"<exact name of the player on my roster to drop, or an');
    lines.push('    empty string>",');
    lines.push('  "confidence":"high|medium|low",');
    lines.push('  "why":"<the news or role reason, dated, with the outlet named>"}],');
    ...
    lines.push('');
    lines.push('RULES FOR THE FIELDS:');
    lines.push('- "swaps" IS THE ANSWER. Each entry is one player off my roster and one off');
    lines.push('  the AVAILABLE list, both named exactly as spelled below.');
    lines.push('- "rank" 1 = the move to make first. Rank a mandated replacement above any');
    lines.push('  optional upgrade.');
    lines.push('- Return NO swap rather than a weak one. An empty "swaps" list with a');
    lines.push('  "summary" saying my roster already beats the wire is a correct answer.');
    lines.push('- NEVER propose adding anybody in the OWNED block — they are not available.');
    lines.push('- A swap must never leave a starting slot with nobody to fill it (do not');
    lines.push('  drop my only tight end for a receiver). Dropping a SPARE at a position');
    lines.push('  I double up on is fine.');
    lines.push('- "adds" is optional and secondary: a player worth watching who is not part');
    lines.push('  of a swap you are recommending. The app reads "swaps" first.');
    lines.push('- Prefer players from the AVAILABLE block. You may name at most 2 who are');
    lines.push('  NOT in it if the news is strong; mark those "confidence":"low" so the app');
    lines.push('  can warn that it cannot verify they are free in this league.');
    ...
    lines.push('- Never invent news, a stat line, or a point edge. If you found nothing on');
    lines.push('  a player, do not recommend him.');
    return lines.join('\n');
  }
