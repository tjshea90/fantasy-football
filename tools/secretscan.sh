...
# WHY THIS EXISTS
# ---------------
# autosave.sh commits with `git add -A` and pushes to a PUBLIC GitHub repo
# without a human looking at the diff first. That is the whole point of it —
# and it is also exactly how a pasted API key ends up permanently in a public
# history. Once pushed, rotating the key is the only real remedy; deleting the
# commit is not, because GitHub keeps unreachable objects reachable by SHA.
#
...
# Test fixtures are not credentials. tools/test_locks.js asserts that the API
# key is redacted from an export, so it necessarily CONTAINS a key-shaped
# string ('sk-ant-api03-SECRET-DO-NOT-EXPORT'). Matching it would wedge every
# checkpoint the moment anyone edits near that line — the exact false-positive
# failure this file's header warns about. Two independent guards:
#   1. length — a real sk-ant key carries ~95 chars after the prefix, fixtures
#      are short, so the threshold below separates them on its own
