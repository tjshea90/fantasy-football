...
/* 2026-09-18: doSync now reads `syncedWeek` (a value captured once at the top
   of the function, named to avoid colliding with the unrelated top-level
   syncWeek() button handler) rather than the mutable module-level `week`
   throughout — see doSync's own comment for why (a concurrent NFL-week
   auto-advance could otherwise file this exact box score under the wrong
   week). */
ok(/if \(window\.Gamelog\) \{ try \{ Gamelog\.ingestEvent\(syncedWeek, g, r\); \} catch \(e\) \{ \} \}/.test(ui),
