# CHECKPOINT 101 — read me first, then TASKS.md

**Written:** 2026-09-23T04:21:09Z · **version:** 8.3 · **tests:** all 22 suites green

## Just done
Code for A (Roster PROJ + AVG column; new Store.playerAvg), B (Live: 'p 14.2' under each not-yet-played starter's 0.0; weekProjById shared with projectedFinish) and E (neutral ⋯ button -> rosterRowMenu dialog: Cancel / Stats / Drop-with-confirm) written; all suites green (test_boot lineupDetail pin updated for the new param). Boxes NOT ticked yet — tests pending.

## Do this next
C: merge Advice into Lineups (index.html nav, tabList, lastTab 'advice' migration, goTab('advice') call sites, test TAB_NAMES in test_tabsafety/test_lifecycle). Then D (sim.js power/playoff card on Data->League). Then new tools/test_picks.js covering A-E via the tabsafety-style DOM harness (render into ids.view and walk children by className); add it to MANIFEST.txt.
